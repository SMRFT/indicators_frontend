...
const MICURawData = ({ showHeading = true }) => {
  ...
  const [isSubmitting, setIsSubmitting] = useState(false); // ✅ Added for disabling
  ...
  useEffect(() => {
    if (selectedDate) {
      ...
      setFormData((prevFormData) =>
        prevFormData.map((patient, index) =>
          index === 0 ? { ...patient, selectedDate: adjustedDate.toISOString().split('T')[0] } : patient
        )
      );
    }
  }, [selectedDate]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.checkValidity() === false) {
      ...
      try {
        ...
        const response = await fetch(`${IndicatorBaseUrl}MICURawData/`, {
          method: 'POST',
          headers: {
    ...
    <Container className="RawData">
      <div>
        ...
        <Form noValidate validated={validated} onSubmit={handleSubmit}>
          ...
          <Row>
            <Col xs="2">
              <Form.Label htmlFor="noOfPatients" style={{ fontWeight: 'bold' }}>
                No of Patients
              </Form.Label>
            </Col>
            ...
            <thead>
              <tr>
                ...
                {[...Array(parseInt(noOfPatients) - 1 || 0)].map((_, index) => (
                  <th
                    style={{ backgroundColor: 'rgb(149,188,176)', color: '#FFFFFF' }}
                    key={index + 2}
                  >
                    Patient {index + 2}
                  </th>
