...
const OPDRawData = ({ showHeading = true }) => {
    ...
    const patientFields = [
        ...
        { key: 'patientArrivedAtOPDInTime', label: 'Patient Arrived At OPD In Time' },
        { key: 'consultantBeginsAssessmentConsultationTime', label: 'Consultant Begins The Assessment Consultation Time' },
        { key: 'waitingTimeForOutPatientConsultation', label: 'Waiting Time For Out Patient Consultation Average Time' },
        { key: 'reasonForDelay', label: 'Reason For Delay' },
        { key: 'prioritizeAccordingToClinicalNeeds', label: 'Prioritize According To Clinical Needs' },
        { key: 'StaffName', label: 'Staff Name - Id No' },
    ...
    const handlePatientCountChange = (e) => {
        ...
        const patients = Array.from({ length: value }, () => ({
            ...
            status: '',
            diagnosis: '',
            patientArrivedAtOPDInTime: '',
            consultantBeginsAssessmentConsultationTime: '',
            waitingTimeForOutPatientConsultation: '',
            reasonForDelay: '',
            prioritizeAccordingToClinicalNeeds: '',
            StaffName: '',
            inchargeStaffName: '',
        }));
    ...
    const handleSubmit = async (e) => {
        ...
        if (form.checkValidity() === false) {
            ...
            try {
                ...
                const response = await fetch(`${IndicatorBaseUrl}OPDRawData/`, {
                    method: 'POST',
                    headers: {
    ...
    return (
        <Container className="RawData">
            <div>
                {showHeading && <h2 className="text-center">OPD RawData</h2>}
